module ApplicationHelper
  ...
  def likes_buttons(comment, user)
    if user.nil?
      ...
      link_to("".html_safe, like_comment_path(comment), :remote => true, :class => "thumbs-up-hov") + "&nbsp;&nbsp;".html_safe + link_to("".html_safe, undislike_comment_path(comment), :remote => true, :class => "thumbs-down") + ((comment.dislikes.size.zero? ? "&nbsp;&nbsp;&nbsp;" : "&nbsp;#{comment.dislikes.size}&nbsp;").html_safe)
    else
