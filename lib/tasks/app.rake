namespace :app do
  ...
  task :update_database_with_tvdb, :activity, :update_type, :needs => :environment do |t, args|
    ...
    puts "#{series_updated.size} series updated."
    
    episodes_updated = { :new => [], :updated => []}
    episodes_ids.each_with_index do |episode_id, index|
      ep = tvdb.get_episode_by_id(episode_id)
      serie = Series.find_by_tvdb_id(ep.series_id)

      puts "Processing episode #{episode_id} (#{index + 1}/#{episodes_ids.size})"
      
      # Only process the episode if the serie is saved in the database
      unless serie.nil?
        puts "Series found! #{serie.name}"
        episode = Episode.find_by_tvdb_id(episode_id)
        
        # The episode does not exists
        if episode.nil? 
          puts "The episode has never been saved in the database. This is a new episode! Check for the season"
          season = Season.find_by_tvdb_id(ep.season_id.to_i)
          ...
          episode = season.episodes.build(:tvdb_id => ep.id, :number => ep.number,
                                :name => ep.name, :description => ep.overview,
                                :director => ep.director, :writer => ep.writer,
                                :first_aired => ep.air_date, :poster_url => ep.thumb)
          puts "Processing new episode #{serie.name} S#{season.number.to_s.rjust(2, '0')}E#{season.number.to_s.rjust(2, '0')}"
          episode.save
          episodes_updated[:new] << episode
        else
          puts "Episode found. #{serie.name} S#{episode.season.number.to_s.rjust(2, '0')}E#{episode.number.to_s.rjust(2, '0')}. Updating."
          episode.update_attributes(:tvdb_id => ep.id, :number => ep.number,
                                    :name => ep.name, :description => ep.overview,
                                    :director => ep.director, :writer => ep.writer,
