def get_db_settings
  database_url = ENV['DATABASE_URL'] || 'postgres://username:password@localhost/milnu_development'
...
def pg_dump(filename)
  ...
  `pg_dump -i -h #{settings[:host]} -U #{settings[:username]} -F c #{settings[:db_name]} > "tmp/#{filename}.pgdump"`
  filename
end
...
def connect_s3!
  s3_config = YAML::load(IO.read(File.join(Rails.root, "config", "amazon_s3.yml")))[Rails.env]
